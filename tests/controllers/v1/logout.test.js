...
describe('logout', () => {
    ...
    describe('requiresAuth middleware', () => {
        it('should fail if the access token is invalid', async () => {
            const response = await request(app).post('/v1/logout').set('Authorization', 'Bearer invalid.token').send().expect(401);
    ...
    it('should logout a user successfully', async () => {
        ...
        const response = await request(app).post('/v1/logout').set('Authorization', `Bearer ${accessToken}`).send().expect(200);
